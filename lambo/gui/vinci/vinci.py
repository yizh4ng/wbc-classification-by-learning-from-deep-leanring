...
class DaVinci(Board, Mind):
  ...
  def _register_events_for_cursors(self):
    ...
    self.bookmarks = [[0],[0]] # [ object_cursors_list, layer_cursors_list ]
    self.jump_object_curosr = 0
    self.jump_layer_curosr = 0

    def _jump_cursors(j, k):
      assert j == 0 or k == 0
      if j != 0:
        self.jump_object_curosr += j
        self.object_cursor = self.bookmarks[0][self.jump_object_curosr % len(self.bookmarks[0])]
      if k != 0:
        self.jump_layer_curosr += k
        self.layer_cursor = self.bookmarks[1][self.jump_layer_curosr % len(self.bookmarks[1])]

    self.state_machine.register_key_event('J', lambda: _jump_cursors(1, 0))
    self.state_machine.register_key_event('K', lambda: _jump_cursors(-1, 0))
    self.state_machine.register_key_event('L', lambda: _jump_cursors(0, 1))
    self.state_machine.register_key_event('H', lambda: _jump_cursors(0, -1))

  def register_bookmarks(self, n, is_obj=True):
    if is_obj:
      self.bookmarks[0].append(n)
    else:
      self.bookmarks[1].append(n)
