...
class Board(Nomear):
  ...
  def __init__(self, title=None, height=5, width=5):
    ...
    self._color_limits = [[None, None]] * 99
    self._cmap = None
  ...
  def imshow_pro(
    ...
    self.imshow(x=x, ax=self.axes, title=title if self._show_title else None
                , norm=norm, aspect=aspect, interpolation=interpolation, alpha=alpha,
                vmin=self._color_limits[self.layer_cursor][0],
                vmax=self._color_limits[self.layer_cursor][1],
                color_bar=self._color_bar, k_space=self._k_space,
                log=self._k_space_log, cmap=cmap,
  ...
  def set_clim(self, vmin: float = None, vmax: float = None):
    self._color_limits[self._layer_cursor] = [vmin, vmax]
    self._draw()
  clim = set_clim
  ...
  def export(self, which: str = None, fmt: str = 'png',
             fps: float = 2, cursor_range: str = None,
             path: str = None, n_tail: int = 0):
