...
def _pad_edge_mean(y, pad_width, n_edge=1, axis=-1):
  """
  ...
  if pad_width == 0:  # No padding
    ...
    y_pad[tuple(y_pad_slice_vec_low)] += y[tuple(y_slice_vec_low)].mean(
      axis=axis, keepdims=True)
    y_pad[tuple(y_pad_slice_vec_high)] += y[tuple(y_slice_vec_high)].mean(
      axis=axis, keepdims=True)
  else:
...
def kkrelation(bg, cri, conjugate=False, phase_offset=0.0, norm_to_nrb=True,
               pad_factor=1, n_edge=1,
               axis=-1, no_iter=False, bad_value=1e-8, min_value=None,
               hilb_kwargs={}, **kwargs):
  """
  ...
  hilb_kwargs.update(
    {'pad_factor': pad_factor, 'n_edge': n_edge, 'axis': axis})
  ...
  if (ratio.ndim == 3) & ((axis == -1) | (axis == 2)) & (not no_iter):
    ...
    for num in range(ratio.shape[0]):
      ph[num, ...] = _np.exp(1j * (hilbertfft(0.5 * _np.log(ratio[num, ...]),
                                              **hilb_kwargs) + phase_offset))
  else:
    ph = _np.exp(
      1j * (hilbertfft(0.5 * _np.log(ratio), **hilb_kwargs) + phase_offset))
...
def hilbertfft(y, pad_factor=1, n_edge=1, axis=-1, copy=True, bad_value=1e-8,
               min_value=None, return_pad=False, **kwargs):
  """
  ...
  """
  y_pad, window = _pad_edge_mean(y, pad_factor * y.shape[axis], n_edge=n_edge,
                                 axis=axis)
...
if __name__ == '__main__':  # pragma: no cover
  ...
  p.paint_samples()
  x = p.physics_based_fringe
  bg = np.zeros(ImageConfig['img_size'])
  p.img = bg
  bg = p.physics_based_fringe

  da = DaVinci()
  da.objects = [x, np.angle(kkrelation(np.fft.fftshift(np.fft.fft2(x)),
                                       np.fft.fftshift(np.fft.fft2(bg))))]
